...
| S2-F02 reproducibility | Added project-local Supabase CLI, Docker pgTAP suite and CI for both upgrade and clean paths. | `package.json`, lockfile, workflow, 141 passing assertions |
...
| S2-F06 live matrix | Expanded from the earlier manual sample to a 141-assertion, two-principal suite covering all 16 private tables and special boundaries. | `supabase/tests/stage2_security_and_lifecycle.test.sql` |
...
- Database: 141/141 pgTAP assertions passed; lint returned zero errors.
